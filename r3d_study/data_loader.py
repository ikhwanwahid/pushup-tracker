...
resolves video paths, and optionally extracts YOLO keypoints
for the cropped dataset.
"""
...
def load_annotations(
    ...
    Expected columns:
        ...
        [optional: shallow, partial_ext, hip_sag, hip_pike, uncontrolled,
         incomplete, annotator, notes, ...]
    ...
    Returns:
        List of rep dicts, each with:
            - video_id: filename without extension (for CV splitting)
...
def extract_keypoints(
    ...
    Runs YOLO pose estimation on each unique video and saves
    keypoints as {video_id}.npy in output_dir.
    ...
    for vid_id, video_path in sorted(videos.items()):
        ...
        for _ in range(total_frames):
            ...
            if result.keypoints is not None and len(result.keypoints):
                ...
                native_kps = kps_data[best_idx]  # (17, 3)
...
def attach_keypoints(
    ...
    cache = {}  # video_id -> full keypoint array
